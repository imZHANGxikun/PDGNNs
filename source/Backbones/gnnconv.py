...
class CustomDecoupledSGC_layer(nn.Module):
    def __init__(self, args):
        super().__init__()
        h_dims, dropout, bias, k, alpha, batch_norm = args.CustomDecoupledSGC_args.values()
        self.neighbor_agg = CustomSGC_Agg(k=k)
        self.feat_trans_layers = nn.ModuleList()
        self.bns = nn.ModuleList()
        if len(h_dims) > 0:
            self.feat_trans_layers.append(nn.Linear(args.d_data, h_dims[0]))
            #self.bns.append(nn.BatchNorm1d(h_dims[0]))
            for i in range(len(h_dims) - 1):
                self.feat_trans_layers.append(nn.Linear(h_dims[i], h_dims[i + 1]))
             #   self.bns.append(nn.BatchNorm1d(h_dims[i + 1]))
            self.feat_trans_layers.append(nn.Linear(h_dims[-1], args.n_cls))
        elif len(h_dims) == 0:
            self.feat_trans_layers.append(nn.Linear(args.d_data, args.n_cls))
        else:
    ...
    def feat_trans(self, x, cls=None):
    ...
    def forward(self, graph, x, tasks=None):
